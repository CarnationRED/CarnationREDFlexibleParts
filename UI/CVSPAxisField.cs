...
namespace CarnationVariableSectionPart.UI
{
    ...
    public class CVSPAxisField : MonoBehaviour
    {
        ...
        public float Max { get => max; set => slider.maxValue = max = Mathf.Max(Min, value); }
        public float Min { get => min; set => slider.minValue = min = Mathf.Min(Max, value); }
        public float Value
        {
            get => slider.value;
                        ...
                        if (Time.unscaledTime - downTime < 0.5f)
                            if (((Vector2)Input.mousePosition - downPos).sqrMagnitude < 100f)
                            {
                                ...
                                if (RectTransformUtility.RectangleContainsScreenPoint(t, Input.mousePosition, null))
                                {
                                    ...
                                    sliderInteractive = slider.interactable;
        ...
        private bool sliderInteractive;
        private float lastUpdate;

        private void Update()
        {
            if (Time.unscaledTime - lastUpdate > 0.08f)
            {
        ...
        private void OnEndEdit(string arg0)
        {
            ...
            if (sliderInteractive) slider.interactable = true;
            IFActivated = false;
        ...
        public void UpdateRGBBackgrrounds()
        {
